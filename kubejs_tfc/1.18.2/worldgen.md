...
- 2nd argument: A consumer with several methods:
    ...
    - `.density(number)`{: .language-kube-18 }: Accepts a number, {% in_unit %}, and sets the density of the vein, defaults to 0.2
    ...
    - If the type is `pipe` the following methods are also available:
        ...
        - `.sign(number)`{: .language-kube-18 }: Accepts a number, {% in_unit %}, and sets the sign of the slant, defaults to 0.5
...
- `.flatness(number)`{: .language-kube-18 }: Accepts a number, {% in_unit %}. It describes the how many solid blocks, as a percentage the surrounding area must contain. Defaults to 0.5
...
- A number, {% in_unit %}, representing the distance from the center of a volcano that this position must be in order to generate. 1 is the maximum radius of the volcano
