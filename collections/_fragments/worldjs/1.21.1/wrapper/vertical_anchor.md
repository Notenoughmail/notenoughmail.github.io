...
- A map:
    ...
    - If the map contains an `'absolute'`{:.s} field, an `absolute` anchor is created
        - The `absolute` object will be parsed as a number to use as the value of the anchor
    - If the map contains a `'above_bottom'`{:.s} field, a `above_bottom` anchor is created
        - The `above_bottom` object will be parsed as a number to use as the value of the anchor
    - if the map contains a `'below_top'`{:.s} field, a `below_top` anchor is created
