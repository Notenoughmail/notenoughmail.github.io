...
- A map:
    ...
    - If the map contains a `'trapezoid'`{:.s} field, a `minecraft:trapezoid` height provider will be created
        - The `trapezoid` object is expected to be a map with the following fields
            - `'min'`{:.s}: A [[ vertical_anchor ]], the minimum height
            - `'max'`{:.s}: A [[ vertical_anchor ]], the maximum height
            - `'plateau'`{:.s}: An `int`{:.p}, the range in the middle of the trapezoid distribution that has a uniform distribution
                - Optional, defaults to `0`{:.n}
    - If the map contains a `'weighted'`{:.s} field, a `minecraft:weighted_list` height provider will be created
        - The `weighted` object is expected to a list of height provider [[ weighted_value ]]s
    - If the map contains a `'biased'`{:.s} field, a `minecraft:biased_to_bottom` height provider will be created
