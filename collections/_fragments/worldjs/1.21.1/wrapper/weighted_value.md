...
Weighted values are used to describe arbitrary weighted collections in a manner that is script-friendly. They are *always* made as part of a list which will never accept null or undefined values. Weights less than `1`{:.n} are also considered invalid and will emit an exception if encountered
...
- A object with `'weight'`{:.s} and `'value'`{:.s} fields:
