...
- `.extraDrop(resultItem: ItemStackProvider)`{: .language-kube-21 #{{ page.anchor }}-extra-drop }: A simple alias to [`.resultItem(...)`{:.language-kube-21}](#{{ page.anchor }}-result-item){:.preserve-color} meant to disambiguate `resultItem`{:.v} from `result`{:.v}
