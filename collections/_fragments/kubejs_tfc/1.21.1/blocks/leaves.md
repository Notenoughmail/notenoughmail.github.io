...
- `.fallenLeaves(fallenLeaves: @Nullable Consumer<FallenleavesBlockBuidler>)`{: .language-kube-21 #leaves-fallen-leaves }: Set the properties of the fallen leaves associated with the leaves. `null`{:.p} is accepted to have to fallen leaves. Accepts a consumer with the following methods
...
- `.twig(twig: Holder<Block>)`{: .language-kube-21 #leaves-twig }: Set the twig block to place when the leaves are broken
- `.autumnIndex(index: int)`{: .language-kube-21 #leaves-autumn-index }: Set the vertical coordinate, {% in_range 0,255 %} on TFC's `foliage_fall` colormap the leaves use for dynamic tinting
- `.seasonalColors()`{: .language-kube-21 #leaves-seasonal-colors }: Make the dynamic tinting of the leaves change with the seasons
- `.noDynamicTinting()`{: .language-kube-21 #leaves-no-dynamic-tinting }: remove all dynamic tinting that is applied to the leaves by default
- `.extendedProperties(extendedProperties: Consumer<ExtendedProperties>)`{: .language-kube-21 #leaves-extended-properties }: Set the [extended properties](#extended-properties) of the leaves block
