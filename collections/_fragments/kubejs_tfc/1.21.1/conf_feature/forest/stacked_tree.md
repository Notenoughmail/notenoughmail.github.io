...
- `.layers(layers: List<StackedTreeLayer>)`{: .language-kube-21 #{{ page.anchor }}-layers }: The layers making up a tree. Each layer can be made as an object with the fields
    - `templates: List<ResourceLocation>`{:.language-kube-21}: The id of the layer's structure, located at `data/<namesapce>/structures/<path>.nbt`
        - [[ required ]]
    - `minCount: int`{:.language-kube-21}: The minimum number of structures from the layer to use
        - **Must** be <= `maxCount`{:.v}
    - `maxCount: int`{:.language-kube-21}: The maximum number of structures from the layer to use
        - **Must** be >= `minCount`{:.v}
